...
class Agent_distributed_dtde(Agent_distributed):
    ...
    def e_greedy_action(self, state, meta_agent, random_seed):
        ...
        if random.random() > self.epsilon(self.epsilon_frame_idx):
            ...
            meta_explore = True
            action = meta_agent.e_greedy_action(state, random_seed * 2)
        # random.seed(None)

        return action, meta_explore
    ...
    def perceive(self, state, action, reward, next_state, will_update):
...
class MetaAgent(Agent_distributed_dtde):
    def __init__(self, config: Config, node_id):
        super().__init__(config, node_id)

        self.epsilon = lambda frame_idx: config.META_EPSILON_END \
                                         + (config.META_EPSILON_START - config.META_EPSILON_END) \
                                         * math.exp(-1 * frame_idx / config.EPSILON_DECAY)

        if os.path.exists(f'{config.DISTRIBUTED_LOAD_MODEL_PATH}/../node_{self.node_id}_meta_model_win_len_{self.time_step}.pth'):
            logging.info("reading exist meta agent")
            self.load_model(config.DISTRIBUTED_LOAD_MODEL_PATH)

    def e_greedy_action(self, state, random_seed):
        self.epsilon_frame_idx += 1
        # random.seed(random_seed)
        if random.random() > self.epsilon(self.epsilon_frame_idx):
            # logging.info(f"meta agent {self.node_id} get action for network")
            with torch.no_grad():
                state_1 = torch.tensor([state[0]], device=self.device, dtype=torch.float32)
                state_2 = torch.tensor([state[1]], device=self.device, dtype=torch.float32)
                q_vals = self.current_net(state_1, state_2)
            ...
            # logging.info(f"meta agent {self.node_id} get action for random")
            # random.seed(None)
    ...
    def greedy_action(self, state):
        # logging.info('agent explore with meta policy ====')
        with torch.no_grad():
            state_1 = torch.tensor([state[0]], device=self.device, dtype=torch.float32)
            state_2 = torch.tensor([state[1]], device=self.device, dtype=torch.float32)
            q_vals = self.current_net(state_1, state_2)
    ...
    def save_current_q_network(self, save_path):
        torch.save(self.target_net.state_dict(), f"{save_path}/../node_{self.node_id}_meta_model_win_len_{self.time_step}.pth")

    def load_model(self, save_path):
        self.target_net.load_state_dict(torch.load(f"{save_path}/../node_{self.node_id}_meta_model_win_len_{self.time_step}.pth"))
