...
class Flow:
    ...
    def __init__(self, config:Config, flow_id):
        # Flow.total_pair_count += 1
        self.flow_id = flow_id
        self.node_id = int( (flow_id-1) / config.HOLD_FLOW_NUMBER)
        # traffic information
        self.offset = 0
        self.inter_period = 0
        self.deadline = 0
        ...
        self.channel_prob = 0
        # packet buffer
        self.packet_buffer = PacketBuffer(self.flow_id)
        # 统计信息
    ...
    def initialize(self, config, traffic_id):
        ...
        self.stats_slot_begin = config.INITIAL_PHASE_TIME + 1
        # 流量信息
        self.offset = config.OFFSET[traffic_id][self.flow_id-1]
        self.inter_period = config.INTER_PERIOD[traffic_id][self.flow_id-1]
        self.deadline = config.DEADLINE[traffic_id][self.flow_id-1]
        self.arrival_prob = config.ARRIVAL_PROB[traffic_id][self.flow_id-1]
        self.channel_prob = config.CHANNEL_PROB[traffic_id][self.flow_id-1]
