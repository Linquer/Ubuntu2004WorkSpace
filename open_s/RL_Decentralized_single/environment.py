...
class Environment:
    ...
    def get_reward(self, actions, expire_list, time_slot, channel_state):
        ...
        # '''reward part 1: collision punish and send packet reward'''
        # if send_node_id == -1 and send_flow_id == -1:
        #     for action in actions:
        #         if action != 0:
        #             # reward_distributed[self.flows [action - 1].node_id] += \
        #             #     -(self.flows[action - 1].channel_prob * self.collision_punish_ratio(self.collision_punish_frame_idx))
        #             # self.collision_punish_frame_idx += 1
        #             reward_distributed[self.flows[action - 1].node_id] += \
        #                 -(self.flows[action - 1].channel_prob * 0.1)

        if send_node_id != -1 and send_flow_id != -1 and channel_state == CHANNEL_STATE.ONE_PACKET:
            # for idx in range(self.node_number):
            #     reward_distributed[idx] += self.flows[send_flow_id - 1].channel_prob
            #
            for idx in range(self.node_number):
                reward_distributed[idx] += 1.0
            # reward_distributed[send_node_id] += self.flows[send_flow_id-1].channel_prob
