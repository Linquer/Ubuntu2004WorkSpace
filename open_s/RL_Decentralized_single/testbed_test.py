...
def parse_args():
    ...
    parser.add_argument('--model_suffix', type=str, default="platform_model")
    parser.add_argument('--traffic_exp_name', type=str, default="flow_10")
...
if __name__ == '__main__':
    ...
    model_path = f'./{exp_prefix}model/{0}_to_{99}_{model_suffix}/{traffic_exp_name}'
    os.makedirs(model_path, exist_ok=True)
    config_logging(file_name=f'{model_path}/test_log.log')
    logging.info(f"pid : {os.getpid()}")

    throughput_res = []
    for traffic_base in range(0, 10):

        traffic_id_begin = traffic_base * 10
        traffic_id_end = traffic_base * 10 + 9
        ...
        for traffic_id in range(traffic_id_begin, traffic_id_end + 1):
            ...
            throughput_res.append(present_timely_throughput)
    ...
    os.makedirs("./data/RL_Decent_simu_res", exist_ok=True)
    np.save(f"./data/RL_Decent_simu_res/throughput_node_{flow_num}_simu.npy", throughput_res)
