...
def parse_args():
    ...
    parser.add_argument('--traffic_id_end', type=int, default=2)
    parser.add_argument('--model_suffix', type=str, default="gen_local_test")
    parser.add_argument('--traffic_exp_name', type=str, default="flow_8/group_0")
    ...
    for traffic_id in range(traffic_id_begin, traffic_id_end + 1):
        ...
        if args.train:
            ...
            for episode in range(1, config.EPISODE_TIMES + 1):
                ...
                for time_slot in range(1, config.SIMULATION_TIMES + 1):
                    action_distributed = [0 for _ in range(config.NODE_NUMBER)]

                    for idx in range(config.NODE_NUMBER):
                        action = agent_distributed[idx].e_greedy_action(
                ...
                if episode % config.TEST_EPISODE_TIMES == 0:
                    ...
                    logging.info(f"({episode}) timely throughput : {present_timely_throughput},"
                                 ...
                                 f" action cnt : {action_cnt}, total time : {total_train_time}, avg time: {total_train_time / episode}")
                    ...
                    if present_timely_throughput > best_timely_throughput:
                        ...
                        for node_id in range(config.NODE_NUMBER):
                            agent_distributed[node_id].save_current_q_network(
                                save_path=config.DISTRIBUTED_LOAD_MODEL_PATH)
        ...
        '''-------------------------plot phase-------------------------'''
        plot_utils.meta_reward_cnt_for_save(x=plot_eps,
